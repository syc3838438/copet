...
const { resolveAppMode } = require("./app-mode");
...
const STANDALONE_PET_MODE = APP_MODE.standalonePet;
...
function _installAutoStartHook() {
  if (!APP_MODE.features.hookServer) return false;
...
// ── Theme loader ──
function createNoopAgentRuntime() {
  return {
    startMonitorForAgent() {},
    stopMonitorForAgent() {},
    syncIntegrationForAgent() { return false; },
    repairIntegrationForAgent() { return false; },
    stopIntegrationForAgent() { return false; },
    clearSessionsByAgent() { return 0; },
    dismissPermissionsByAgent() { return 0; },
    getCodexSubagentClassifier() { return null; },
...
function createNoopServer() {
  return {
    startClaudeSettingsWatcher() { return false; },
    stopClaudeSettingsWatcher() { return false; },
    repairRuntimeStatus() {
      return { status: "disabled", message: "Hook server is disabled in standalone pet mode" };
    },
    cleanup() {},
  };
}
...
function createNoopUpdater() {
  return {
    setupAutoUpdater() {},
    checkForUpdates() {},
    getUpdateMenuItem() { return null; },
    getUpdateMenuLabel() { return null; },
  };
}

function createNoopRemoteSshRuntime() {
  return {
    cleanup() {},
  };
}

function createNoopRemoteSshIpc() {
  return {
    dispose() {},
  };
}
...
function getRegisteredAgents() {
  if (!APP_MODE.features.agents) return [];
  if (!agentRegistry) {
    agentRegistry = require("../agents/registry");
  }
  return typeof agentRegistry.getAllAgents === "function"
...
if (APP_MODE.features.dashboard) {
...
agentRuntime = APP_MODE.features.agents
  ? require("./agent-runtime-main")({
    getServer: () => _server,
    getStateRuntime: () => _state,
...
const _server = APP_MODE.features.hookServer
  ? require("./server")(_serverCtx)
  : createNoopServer();
const startHttpServer = APP_MODE.features.hookServer ? _server.startHttpServer : () => {};
const getHookServerPort = APP_MODE.features.hookServer ? _server.getHookServerPort : () => 0;
...
function getTelegramApprovalTokenInfo() {
  if (!APP_MODE.features.telegramApproval) return { configured: false, masked: "" };
  const paths = getTelegramApprovalPaths();
...
function getTelegramApprovalStatus() {
  if (!APP_MODE.features.telegramApproval) {
    return {
      status: "disabled",
      reason: "disabled",
      ready: false,
...
function writeTelegramApprovalToken(token) {
  if (!APP_MODE.features.telegramApproval) {
    return { status: "error", message: "Telegram approval is disabled in standalone pet mode" };
  }
  const paths = getTelegramApprovalPaths();
...
async function syncTelegramApprovalSidecar(reason = "settings") {
  if (!APP_MODE.features.telegramApproval) return false;
  const config = getTelegramApprovalPrefs();
  const paths = getTelegramApprovalPaths();
  const token = getTelegramApprovalTokenStatus();
  const ready = telegramApprovalSettings.readiness(config, token);
...
function queueTelegramApprovalSidecarSync(reason) {
  if (!APP_MODE.features.telegramApproval) return Promise.resolve(false);
...
const _menuCtx = {
  ...
  openDashboard: () => {
    if (APP_MODE.features.dashboard) showDashboard();
  },
...
_settingsController.subscribeKey("petBehavior", () => {
  sendToHitWin("theme-config", getHitRendererConfig());
});
...
ipcMain.on("settings:renderer-diagnostic", (_event, diagnostic) => {
  try {
    console.warn("Settings renderer diagnostic:", JSON.stringify(diagnostic));
  } catch {
    console.warn("Settings renderer diagnostic:", diagnostic);
  }
});
...
const _updater = APP_MODE.features.updater
  ? require("./updater")(_updaterCtx)
  : createNoopUpdater();
const { setupAutoUpdater, checkForUpdates, getUpdateMenuItem, getUpdateMenuLabel } = _updater;

// ── Doctor tab IPC ──
if (APP_MODE.features.doctor) {
...
let _remoteSshRuntime = createNoopRemoteSshRuntime();
let _remoteSshIpc = createNoopRemoteSshIpc();
if (APP_MODE.features.remoteSsh) {
  ...
  _remoteSshIpc = registerRemoteSshIpc({
    ipcMain,
    settingsController: _settingsController,
...
registerSettingsIpc({
  ...
  getAllAgents: () => getRegisteredAgents(),
  checkForUpdates,
...
function createWindow() {
  ...
  hitWin = petWindowRuntime.createHitWindow({
    ...
    hitThemeConfig: getHitRendererConfig(),
    guardAlwaysOnTop,
    onDidFinishLoad: () => {
      sendToHitWin("theme-config", getHitRendererConfig());
  ...
  initFocusHelper();
  startMainTick();
  if (APP_MODE.features.hookServer) startHttpServer();
  startStaleCleanup();
...
if (!gotTheLock) {
  ...
  app.whenReady().then(() => {
    ...
    if (APP_MODE.features.agents) {
      agentRuntime.startCodexLogMonitor();
    }

    // Auto-install VS Code/Cursor terminal-focus extension
    if (APP_MODE.features.terminalFocusInstall) {
