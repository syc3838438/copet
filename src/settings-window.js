...
function createSettingsWindowRuntime(options = {}) {
  ...
  function open() {
    ...
    const additionalArguments = Array.isArray(options.additionalArguments)
      ? options.additionalArguments.slice()
      : [];
    if (appMode && appMode.standalonePet && !additionalArguments.includes("--standalone-pet")) {
      additionalArguments.push("--standalone-pet");
    }
    ...
    createdWindow.webContents.on("console-message", (_event, level, message, line, sourceId) => {
      if (level >= 2) {
        console.warn(`Settings renderer console[${level}] ${sourceId || ""}:${line || 0} ${message}`);
      }
    });
    createdWindow.webContents.on("did-fail-load", (_event, errorCode, errorDescription, validatedURL) => {
      console.warn(`Settings renderer failed to load ${validatedURL || ""}: ${errorCode} ${errorDescription}`);
    });
    createdWindow.webContents.on("render-process-gone", (_event, details) => {
      console.warn(`Settings renderer process gone: ${details && details.reason}`);
    });
